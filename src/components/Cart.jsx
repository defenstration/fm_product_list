...
const Cart = () => {
    const { cart, showModal } = useContext(CartContext)
    let total = cart.reduce((sum, product) => sum + product.quantity * product.price, 0)
    let num_items = cart.reduce((sum, product)=> sum + product.quantity, 0) 

    return (
        <div className = "cart-wrapper">

            <div className = "cart-header">
            <h4 className = "text-2 text-red">
                Your Cart ({num_items > 0 ? num_items : "<-- Quantity -->"})
            </h4>
            </div>
            {cart.length === 0 ? (
                <div className = "empty-cart flex">
                    ...
                    <p className = "cart-contents">Your added items will appear here</p>
                </div>) :
                (<div className = "full-cart flex">  
                    {cart.map((product) => {
                    return <CartEntry key = {product.name} {...product} />})}
                    <p>Order Total: ${total.toFixed(2)}</p>
                    <div className = "carbon-neutral flex text-4">
                    ...
                    <button className = "confirm-btn text-4" onClick={showModal}>Confirm Order</button>
                </div>)
                }  
        </div> 
    )
